...
TEST_CASE("dielectric_2", "[dielectric]")
{
    int nx{200};
    int ny{100};
    int ns{10};

    std::ofstream os("8_dielectric_2.ppm");
    os << "P3\n"
       << nx << ' ' << ny << "\n255\n";
...
TEST_CASE("dielectric_3", "[dielectric]")
{
    int nx{200};
    int ny{100};
    int ns{10};

    std::ofstream os("8_dielectric_3.ppm");
    os << "P3\n"
       << nx << ' ' << ny << "\n255\n";
    ...
    camera cam(vec3{-2, 2, 0.3}, vec3{0, 0, -1}, vec3{0, 1, 0}, degree_to_radian(90), float(nx) / ny);
    for (int j = ny - 1; j >= 0; j--)
        for (int i = 0; i < nx; i++)
        {
            vec3 col(0, 0, 0);
